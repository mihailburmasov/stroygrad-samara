...
const iconSvg = (size) => `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="-6 -4 52 52"><rect x="-6" y="-4" width="52" height="52" rx="10" fill="#0a0a0b"/>${mark(AMBER)}</svg>`;

fs.mkdirSync('public/img', { recursive: true });
fs.writeFileSync('public/favicon.svg', `<svg xmlns="http://www.w3.org/2000/svg" viewBox="-6 -4 52 52"><rect x="-6" y="-4" width="52" height="52" rx="10" fill="#0a0a0b"/>${mark(AMBER)}</svg>`);
...
// OG-картинка
const W = 1200, H = 630;
const photo = await sharp('public/img/hero/collage-home.webp').resize(860, H, { fit: 'cover', position: 'right' }).flatten({ background: '#0a0a0b' }).toBuffer();
const fade = Buffer.from(`<svg width="${W}" height="${H}"><defs><linearGradient id="g" x1="0" x2="1"><stop offset="0" stop-color="#0a0a0b" stop-opacity="1"/><stop offset=".42" stop-color="#0a0a0b" stop-opacity=".92"/><stop offset="1" stop-color="#0a0a0b" stop-opacity=".25"/></linearGradient></defs><rect width="${W}" height="${H}" fill="url(#g)"/></svg>`);
...
<text x="72" y="345" font-family="Arial, Helvetica, sans-serif" font-size="30" fill="#e5ac24">Строительная компания · Самара</text>
...
await sharp({ create: { width: W, height: H, channels: 3, background: '#0a0a0b' } })
